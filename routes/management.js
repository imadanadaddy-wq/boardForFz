...
const STOCK_THRESHOLDS = {
  fuel: { min: 50,    label: "연료(2000039)" },
  wap:  { min: 50,    label: "재획비(WAP)" },
  ale:  { min: 10000, label: "ale(2002023)" },
};
...
function getStockCounts(items) {
  ...
  return {
    fuel: fuelCount,
    // WAP 환산: 30분(2003611) + 2시간(2003551)x4
...
function checkStock() {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      for (const type of Object.keys(STOCK_THRESHOLDS)) {
        const key   = `${botKey}|${type}`;
        const thr   = STOCK_THRESHOLDS[type];
        const count = counts[type] || 0;

        // 대상 아님(오프라인/비활성) → 발송상태만 정리(해소 알람은 안 보냄)
        if (!eligible) { stockAlertSent.delete(key); continue; }
        ...
        if (count < thr.min) {
          ...
          if (!stockAlertSent.has(key)) {
            ...
            sendDiscord([{
              color: 0xe74c3c,
              title: "📦 재고 부족 경고",
              description: `**${row.ign}** (Lv.${row.level}) — ${thr.label} 재고가 부족합니다`,
              fields: [
                { name: "현재 수량", value: `**${count.toLocaleString()}**`, inline: true },
                { name: "기준치",    value: `${thr.min.toLocaleString()} 미만`, inline: true },
              ],
              footer: { text: "Maple Dash · Stock Monitor" },
              timestamp: new Date().toISOString()
            }], DISCORD_WEBHOOK_STOCK);
            console.log(`[STOCK-ALERT] ${row.ign} ${type} low: ${count} < ${thr.min}`);
          }
        } else {
          ...
          if (stockAlertSent.has(key)) {
            ...
            sendDiscord([{
              color: 0x2ecc71,
              title: "✅ 재고 회복",
              description: `**${row.ign}** (Lv.${row.level}) — ${thr.label} 재고가 충전되었습니다`,
              fields: [
                { name: "현재 수량", value: `**${count.toLocaleString()}**`, inline: true },
